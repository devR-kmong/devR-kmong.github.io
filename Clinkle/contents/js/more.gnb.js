...
$('.toggle-closeBtn, .toggle-menu-list li').click(function(){
    ...
    $('.sub-menu').removeClass('sub-menu-visible')
    $('.toggle-menu-overlay').css('width','100%');
});
...
$(document).ready(function () {        
    $("#check").change(function () {
        if ($("#check").is(":checked")) {
            $('.toggle-menu-overlay').css('width','0%');
            $('.toggle-menu-list').removeClass('t-m-l-visible')
            $('.ri-arrow-down-s-line').removeClass('t-m-i-rotate')
    }
    });
});